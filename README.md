...
Desktop Output also offers one Line or Scatter chart using numeric Workflow Outputs as axes, with optional 1-based Iteration as X for loop results. Charts use committed ResultRows and update during a run, including partial rows retained for inspection after failure. Charts are Desktop presentation only; selections are not stored in Templates, and CSV semantics are unchanged.

...
Input suggestions follow Core scope: loop body steps can read root outputs that exist before the loop and earlier body sibling outputs from the same iteration, plus earlier ordinary variables. A For body can additionally use its enclosing For loop variable; While has no loop variable. Later root steps cannot see body StepOutputs, and a newly introduced For loop variable does not leak (a pre-existing variable of the same name is restored after For), but ordinary variables set in the body remain visible. A row-producing While may execute zero iterations.

Core `execute_workflow_with_events` and `run_workflow_with_events` notify callers of completed `StepExecution` records and committed `ResultRow` values during execution. Desktop Simulation and Live use a Tauri Channel to update Execution Results and the Output table incrementally. Rows are notified only after commit, including each successful row-producing For or While iteration. Partial progress remains available for inspection if the command fails; the final command result is the authoritative `WorkflowRunResult`. Manual CSV export requires a successfully completed run. Pause, hard cancel, and nested loops are not supported.

Desktop optionally streams CSV during Simulation or Live runs; it is off by default. Enable **Stream CSV during run** and optionally choose an output folder (default: `data` under the application directory). Before execution, Desktop creates and flushes the header of a new `YYYY-MM-DD-HH-MM-SS_<workflow-name>.csv` file using UTC+8; collisions receive `-2`, `-3`, and later suffixes without overwriting files. Each `ResultRowCommitted` writes and flushes one record using the same columns and formatting as manual export, without iteration metadata. Failed runs retain their committed rows; graceful Stop needs no special handling. A creation error prevents execution; a later write/flush error stops streaming and is displayed while the workflow continues normally. Manual export still requires authoritative final success and committed rows. Folder selection and streaming status remain Desktop session state, outside Templates. Flush does not guarantee durability against power loss.

Active For / While loops offer **Stop** in the Workflow controls and Output view once body progress is observed. Stop finishes the current iteration without interrupting its steps; a successful iteration commits its row normally, then the targeted loop exits successfully and later root steps continue. Normal Powers cleanup and Worker shutdown still run. A successful final workflow remains eligible for normal manual CSV export. Stop is not a hard cancel.
...
Desktop Charts visualize iterative For/While ResultRows as lines with X fixed to 1-based Iteration. Each panel uses checkboxes to select one or multiple numeric Outputs; the first panel defaults to the first numeric Output, and users can add panels up to a maximum of 8. Charts update from committed rows during execution, and failed runs may retain committed rows for inspection. Chart configuration is Desktop session state only, is not stored in Templates, and leaves CSV semantics unchanged.

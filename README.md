...
While reuses the existing comparison operands and operators, with no equality, boolean trees, nested loops, break, continue, or timeout semantics. In Template schema v1, `max_iterations` is required: a positive integer means a finite limit, explicit `null` means Unlimited, and omitting the field is invalid. A finite limit is represented as:

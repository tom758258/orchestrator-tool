...
test('Page adapter builds only requested series once and isolates Pages', () => {
  const rows = [1, 2].map(value => ({ page: 'A', outputs: [{ name: 'V', value }, { name: 'I', value: value * 2 }] }))
  const original = structuredClone(rows)
  const page = createPageChartData(rows)
  assert.equal(page.rowCount, 2)
  assert.ok(page.iteration instanceof Float64Array)
  assert.deepEqual([...page.iteration], [1, 2])
  assert.equal(page.series.size, 0)
  const voltage = page.getSeries('V')
  assert.ok(voltage instanceof Float64Array)
  assert.equal(page.series.size, 1)
  assert.equal(page.getSeries('V'), voltage)
  assert.deepEqual([...page.getSeries('I')], [2, 4])
  assert.equal(page.series.size, 2)
  const other = createPageChartData([{ page: 'B', outputs: [{ name: 'V', value: 99 }] }])
